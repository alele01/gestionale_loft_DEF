/**
 * Province italiane (sigle a 2 lettere + nome esteso).
 *
 * Lista ufficiale ISTAT al 2026 (107 entries): include le 107 province,
 * città metropolitane e Province autonome di Trento e Bolzano. Le sigle
 * sono usate in fatturazione elettronica e nel campo XML
 * `IndirizzoSede.Provincia` come SOLO 2 caratteri maiuscoli.
 *
 * NOTA: la lista è ordinata alfabeticamente per nome esteso così
 * compare ordinata nella dropdown del form.
 */
...
export const ITALIAN_PROVINCES: ReadonlyArray<ItalianProvince> = [
...
/** Set delle sigle valide, per validazione veloce. */
export const ITALIAN_PROVINCE_CODES: ReadonlySet<string> = new Set(
  ITALIAN_PROVINCES.map((p) => p.code)
);

export function isValidProvinceCode(code: string | null | undefined): boolean {
  if (!code) return false;
  return ITALIAN_PROVINCE_CODES.has(code.toUpperCase().trim());
}
